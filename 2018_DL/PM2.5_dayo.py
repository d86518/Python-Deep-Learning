...
df = pd.read_csv('F:\\DL_2018\PRSA_data_2010.1.1-2014.12.31.csv')
...
df.dtypes
#
#pd.set_option()可讓panda顯示所有column
# - 時資料
...
df.reset_index(drop=True, inplace=True) #讓編號在從0開始編
...
#把每個時間欄位combine後append到新的column
...
scaler_pm25 = MinMaxScaler(feature_range=(0, 1)) #將特徵屬性縮放到1-0之間
#可增強方差小的屬性其穩定性
df['scaled_pm2.5'] = scaler_pm25.fit_transform(np.array(df['pm2.5']).reshape(-1, 1)) # Why reshape? 均一話=>調到0-1之間
