...
@author: Vince
"""
### Case 1: Tuning batch_size and epochs (out of the create_model())
...
def create_model(): # batch_size and epochs are out of the create_model()
    # create model
    model = Sequential()
...
# load dataset
dataset = numpy.loadtxt('F:\\DL\DL_2018\data\pima-indians-diabetes.csv', delimiter=",")
...
### Case 2: Tuning nodes and lrs (inside the create_model())
...
def build_model(nodes=10, lr=0.001): # nodes and lrs are inside the create_model()
    model = Sequential()
...
### Reference:
# Grid Search the number of hidden layers with keras https://stackoverflow.com/questions/47788799/grid-search-the-number-of-hidden-layers-with-keras
